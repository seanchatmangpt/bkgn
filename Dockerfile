...
ENV VIRTUAL_ENV /opt/bkgn-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/bkgn/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/bkgn/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/bkgn/ && touch src/bkgn/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/bkgn/
...
# Expose the app.
ENTRYPOINT ["/opt/bkgn-env/bin/bkgn"]
CMD []
